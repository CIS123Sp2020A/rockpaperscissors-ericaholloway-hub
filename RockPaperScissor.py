...
#create radio buttons labeled Rock Papre Scissors

#create play and quit buttons; ok quit at the bottom frame 

#Have the computer make a random move out of the selections Rock(0), Paper(1), or Scissors (2)
#this will be displayed with player selection as well

#Create if/else statements around whether the player wins or not, attached to text which tells the action
#of their moves
...
class MyGUI:
    ...
    #Create if/else statements around whether the player wins or not, attached to text which tells the action of their moves; message box
        ...
        if ((self.radioVars.get() == 'Rock') and (self.radioVar.get() == 'Scissors')):
            self.resultVar.set('You Lose: Rock crushes Scissors')
        elif ((self.radioVars.get() == 'Scissors') and (self.radioVar.get() == 'Rock')):    
            self.resultVar.set('You Win!: Rock crushes Scissors')   
        elif ((self.radioVars.get() == 'Paper') and (self.radioVar.get() == 'Rock')):
            self.resultVar.set('You Lose: Paper covers Rock')
        elif ((self.radioVars.get() == 'Rock') and (self.radioVar.get() == 'Paper')):
            self.resultVar.set('You Win!:Paper covers Rock')  
        elif ((self.radioVars.get() == 'Scissors') and (self.radioVar.get() == 'Paper')):
            self.resultVar.set('You Lose: Scissors beats Paper')
        else:
            self.resultVar.set('You Win!: Scissors beats Paper')
